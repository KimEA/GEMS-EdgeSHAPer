...
class GCNNWrapper(nn.Module):
    ...
    edge_weight = edge_attr[:, 6]  (Cβ 거리/10)
        - 공유결합:   결합 길이 / 10  (index 3~6 모두 동일값)
        - 비공유결합: 리간드 원자 → 단백질 Cβ 거리 / 10
        Cβ는 사이드체인 첫 번째 원자로, 원논문의 pseudo-atom 간 거리 의도에
        Cα(index 3)보다 더 부합함.
    """
    ...
    def forward(self, graphbatch):
        edge_weight = None
        if graphbatch.edge_attr is not None and graphbatch.edge_attr.shape[1] > 6:
            edge_weight = graphbatch.edge_attr[:, 6]  # Cβ 거리/10
