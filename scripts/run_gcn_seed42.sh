#!/bin/bash
# =============================================================================
# Step A: GEMS 학습 → M sensitivity 검증 → GC_GNN 재학습 → XAI (seed42)
# =============================================================================
# 실행 순서:
#   1. GEMS seed42 학습 (체크포인트 생성)
#   2. M sensitivity 검증 (GEMS 체크포인트 필요)
#   3. GC_GNN seed42 재학습 (Cβ edge_weight 적용)
#   4. 4개 모델 XAI (학습 스킵, 체크포인트 재사용)
#   5. 엣지 거리 정보 추가 (contact_validation 전처리)
#
# 사용법:
...
# ── 환경 확인 ──────────────────────────────────────────────────────────────────
echo "============================================================"
echo "  Step A: GEMS 학습 → M sensitivity → GC_GNN 재학습 → XAI"
...
echo ""
echo "------------------------------------------------------------"
echo "[1/4] GEMS B6AEPL seed42 학습 (~수 시간)"
echo "------------------------------------------------------------"
...
echo ""
echo "[완료] GEMS seed42 학습 완료"
echo "  체크포인트: ${OUTPUT_DIR}/checkpoints/gems_b6aepl_cleansplit_seed42_best.pt"

# ── 2. M sensitivity 검증 (GEMS 체크포인트 필요) ──────────────────────────────
echo ""
echo "------------------------------------------------------------"
echo "[2/4] M sensitivity 검증 (M=50 vs M=100, 10개 샘플, ~1시간)"
echo "------------------------------------------------------------"
...
echo ""
echo "  [확인] 위 판정 결과에서 M=50 사용 가능 여부 확인 후 계속 진행됩니다."
echo ""

# ── 3. GC_GNN seed42 재학습 ───────────────────────────────────────────────────
echo ""
echo "------------------------------------------------------------"
echo "[3/4] GC_GNN Cβ 재학습 (seed=42)"
echo "------------------------------------------------------------"
...
# ── 4. 전체 모델 XAI (seed42, 학습 스킵) ─────────────────────────────────────
echo ""
echo "------------------------------------------------------------"
echo "[4/4] 4개 모델 XAI 실행 (seed=42, M=${M})"
echo "------------------------------------------------------------"
python run_pipeline.py \
    --cleansplit_b6aepl     "${CLEANSPLIT_AEPL}" \
    --cleansplit_b6ae0l     "${CLEANSPLIT_AE0L}" \
...
echo ""
echo "------------------------------------------------------------"
echo "[5/5] 엣지 거리 정보 추가 (contact_validation 전처리)"
echo "------------------------------------------------------------"
